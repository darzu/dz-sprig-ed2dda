...
export function registerControllableSystems(em) {
    ...
    em.registerSystem([ControllableDef, LinearVelocityDef, RotationDef, WorldFrameDef], [InputsDef, PhysicsTimerDef, MeDef], (controllables, res) => {
    ...
    em.registerSystem([ControllableDef, CameraFollowDef], [InputsDef, PhysicsTimerDef, MeDef], (controllables, res) => {
        for (let c of controllables) {
            if (AuthorityDef.isOn(c) && c.authority.pid !== res.me.pid)
                continue;
            if (c.controllable.modes.canTurn)
                quat.rotateX(c.cameraFollow.rotationOffset, c.cameraFollow.rotationOffset, -res.inputs.mouseMovY * c.controllable.turnSpeed);
        }
